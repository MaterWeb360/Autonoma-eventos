...
foreach ($lista as $key => $item) {
    ...
    $ubiimg = elCampo($item, 'ubicacionimg');
    ...
    $fechaimg = fileCampo($item, 'fechaimg');
    $ubiimg = fileCampo($item, 'ubicacionimg');
    $arrowimg = fileCampo($item, 'flecha_img');
    $ubiimg_final = !empty($ubiimg) ? $ubiimg : TEMA_P_IMG . '/icon-place.svg';
    $fechaimg_final = !empty($fechaimg) ? $fechaimg : TEMA_P_IMG . '/icon-clock-v.svg';
    $arrowimg_final = !empty($arrowimg) ? $arrowimg : TEMA_P_IMG . '/flecha-izquierda-2.svg';
            ...
            <div class="webinar_main">
                ...
                <div class="webinar_details">
                    <div class="webinar_detail-item color_secundario">
                        <img src="{$ubiimg_final}" loading="lazy" alt=""
                            class="webinar_detail-icon">
                        <div>{$ubicacion}</div>
                    </div>
                    <div class="webinar_detail-item color_secundario">
                        <img src="{$fechaimg_final}" loading="lazy" alt=""
                            class="webinar_detail-icon">
                        <div>{$horaAMPM}</div>
                    </div>
                </div>
                ...
                <div class="webinar_btn">
                    <a href="{$url}" class="button w-inline-block" target="_blank">
                        ...
                        <img src="{$arrowimg_final}" loading="lazy" alt="">
                    </a>
                </div>
            </div>
        </div>
